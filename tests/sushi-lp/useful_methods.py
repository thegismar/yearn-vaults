...
def stateOfStrat(strategy, lp, sushi):
    print('\n----state of strat----')
    
    deposits, borrows = strategy.getCurrentPosition()
    sushiBal = sushi.balanceOf(strategy)
    print('Sushi:', Wei(sushiBal).to('ether'))
    print('LP:',lp.balanceOf(strategy).to('ether'))
    print('borrows:', Wei(borrows).to('ether'))  
    print('deposits:', Wei(deposits).to('ether'))
    realbalance = lp.balanceOf(strategy) + deposits - borrows
...
def deposit(amount, user, lp, vault):
    print('\n----user deposits----')
    lp.approve(vault, amount, {'from': user})
    print('deposit amount:', amount.to('ether'))
    vault.deposit(amount, {'from': user})    

def withdraw(share,whale, lp, vault):
    balanceBefore = lp.balanceOf(whale)
    ...
    balanceAfter = lp.balanceOf(whale)
